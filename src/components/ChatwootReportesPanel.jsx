...
export default function ChatwootReportesPanel({ cardStyle, sectionTitleStyle }) {
  ...
  const toggleNodo   = (v) => setFiltroNodos(prev => prev.includes(v) ? prev.filter(x => x !== v) : [...prev, v]);
  ...
  function generarPDF() {
    ...
    if (filtroNodos.length > 0) doc.text(`Nodos: ${filtroNodos.map(formatNodo).join(", ")}`, 14, 44);

    autoTable(doc, {
      startY: 50,
      head: [["Canal", "Agente", "Nodo", "Total", "Abiertas", "Resueltas", "Pendientes", "Pospuestas", "% Total"]],
      body: [
        ...agrupado.map(r => [
          r.inbox_name || "—", r.agent_name || "Sin asignar", formatNodo(r.nodo),
  ...
  const MultiSelect = ({ label, options, selected, onToggle, show, setShow, formatOption }) => (
    <div style={{ display: "flex", flexDirection: "column", gap: 4, position: "relative" }}>
      ...
      <div onClick={() => setShow(!show)} style={{ ...inputSt, minWidth: 160, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", userSelect: "none" }}>
        <span style={{ color: selected.length === 0 ? "#94a3b8" : "#1e293b" }}>
          {selected.length === 0 ? "Todos" : selected.length === 1 ? (formatOption ? formatOption(selected[0]) : selected[0]) : `${selected.length} seleccionados`}
        </span>
      ...
      {show && (
        <div style={{ position: "absolute", top: "100%", left: 0, zIndex: 100, background: "#fff", border: "1px solid #e2e8f0", borderRadius: 8, minWidth: 200, boxShadow: "0 4px 16px rgba(0,0,0,0.1)", marginTop: 4, maxHeight: 220, overflowY: "auto" }}>
          ...
          {options.map(opt => (
            <div key={String(opt)} onClick={() => onToggle(opt)}
              ...
              <span style={{ color: "#1e293b" }}>{formatOption ? formatOption(opt) : opt}</span>
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <div style={{ padding: "24px 16px", maxWidth: 1200, margin: "0 auto" }} onClick={() => { setShowInboxDD(false); setShowAgenteDD(false); setShowNodoDD(false); }}>
      ...
      <div style={{ ...cardStyle, marginBottom: 16 }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
          ...
          <MultiSelect label="Nodo"    options={nodos}   selected={filtroNodos}   onToggle={toggleNodo}   show={showNodoDD}   setShow={setShowNodoDD}  formatOption={formatNodo} />
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
        {[
          ...
          { label: "NODOS",      value: nodos.filter(n => n != null).length, color: "#0f172a" },
        ].map((k) => (
          <div key={k.label} style={{ ...cardStyle, flex: 1, minWidth: 90, textAlign: "center", padding: "14px 10px" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            ...
            <tfoot>
              <tr style={{ background: "#f1f5f9", borderTop: "2px solid #e2e8f0", fontWeight: 700 }}>
                <td colSpan={3} style={{ padding: "11px 14px", color: "#1e293b" }}>Total</td>
